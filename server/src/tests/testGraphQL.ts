...
const execute = async (query: string, variables?: any) => {
...
(async () => {
  ...
  if (sendResult?.errors) {
    ...
    console.log(`   ${sender?.username} shows sent requests:`, 
  ...
  if (checkUser1?.errors) {
    ...
    console.log(`✅ ${user1Data?.username} has sent requests:`,
      user1Data?.sentFriendRequests?.length || 0);
  }
  ...
  if (checkRequests?.errors) {
    ...
    if (requests.length > 0) {
      console.log('   From:', requests.map((r: any) => r.username).join(', '));
    }
  }
  ...
  if (checkUser2?.errors) {
    ...
    console.log(`✅ ${user2Data?.username} has received requests:`,
      user2Data?.receivedFriendRequests?.length || 0);
  }
