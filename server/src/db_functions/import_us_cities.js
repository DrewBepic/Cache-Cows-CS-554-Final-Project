...
import { UScities as getUsCitiesCollection } from '../db_config/mongoCollections.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

export async function importCitiesIfEmpty() {
  const getCol = getUsCitiesCollection;
  const collection = await getCol();

  const count = await collection.countDocuments();
  if (count > 0) {
    console.log('US cities already imported');
    return { imported: 0, reason: 'already-populated' };
  }

  const filePath = path.join(__dirname, 'us_cities.json');
  if (!fs.existsSync(filePath)) {
    console.log('us_cities.json not found at', filePath);
