...
export const sendFriendRequest = async (currentUserId, friendUsername) => {
    ...
    const isAlreadyFriend = currentUser.friends.some(id => id.toString() === friend._id.toString());
    if (isAlreadyFriend) {
        throw new Error("Already friends");
    }
    //and make sure we havent alr sent a request
    const hasSentRequest = currentUser.sent_friend_requests.some(id => id.toString() === friend._id.toString());
    if (hasSentRequest) {
        throw new Error("Friend request already sent");
    }
    //now check if we've already received a request from that friend
    const hasReceivedRequest = currentUser.received_friend_requests.some(id => id.toString() === friend._id.toString());
    if (hasReceivedRequest) {
        throw new Error("This user has already sent you a friend request");
    }
    //passed all checks!
    await Promise.all([
        usersCollection.updateOne({ _id: currentUser._id }, { $addToSet: { sent_friend_requests: friend._id } }),
        usersCollection.updateOne({ _id: friend._id }, { $addToSet: { received_friend_requests: currentUser._id } })
    ]);
...
export const acceptFriendRequest = async (currentUserId, friendId) => {
    ...
    await Promise.all([
        usersCollection.updateOne({ _id: currentUser._id }, {
            $pull: { received_friend_requests: friend._id },
            $addToSet: { friends: friend._id }
        }),
        usersCollection.updateOne({ _id: friend._id }, {
...
export const rejectFriendRequest = async (currentUserId, friendId) => {
    ...
    await Promise.all([
        usersCollection.updateOne({ _id: currentUser._id }, { $pull: { received_friend_requests: friend._id } }),
        usersCollection.updateOne({ _id: friend._id }, { $pull: { sent_friend_requests: currentUser._id } })
    ]);
    const updatedUser = await usersCollection.findOne({ _id: currentUser._id });
    const { password, ...userWithoutPassword } = updatedUser;
    return userWithoutPassword;
...
export const removeFriend = async (currentUserId, friendId) => {
    ...
    await Promise.all([
        usersCollection.updateOne({ _id: currentUser._id }, { $pull: { friends: friend._id } }),
        usersCollection.updateOne({ _id: friend._id }, { $pull: { friends: currentUser._id } })
    ]);
    const updatedUser = await usersCollection.findOne({ _id: currentUser._id });
    const { password, ...userWithoutPassword } = updatedUser;
    return userWithoutPassword;
...
export const getFriendRequests = async (userId) => {
    const usersCollection = await users();
    const user = await usersCollection.findOne({ _id: new ObjectId(userId) });
    if (!user || !user.received_friend_requests.length) {
        return [];
    }
...
export const getSentFriendRequests = async (userId) => {
    const usersCollection = await users();
    const user = await usersCollection.findOne({ _id: new ObjectId(userId) });
    if (!user || !user.sent_friend_requests.length) {
        return [];
    }
